<template>
  <div>
    <p v-if="msTime.show" class="countdown_wrapper">
      ...
      <span>{{msTime.hour}}</span><i class="semicolon">{{hourTxt}}</i>
      <span>{{msTime.minutes}}</span><i class="semicolon">{{minutesTxt}}</i>
...
//                    this.secondsFixed ? msTime.seconds = new Number(timeDistance / 1000).toFixed(2) : msTime.seconds = Math.floor( timeDistance / 1000 ).toFixed(0);
      ...
      end_message(){
        ...
        this.$emit('end_callback', this.index);
      }
    }
  }
</script>
